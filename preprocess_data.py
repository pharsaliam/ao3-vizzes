...
from utils import (
    logger, WORKS_CSV, TAGS_CSV, WORKS_TAGS_PARQUET,
    TAGS_AGGREGATED_LOCATIONS, WORKS_WITH_FANDOM_LOCATIONS,
    FANDOM_WORKS_COUNT_PARQUET, MINIMUM_WORK_COUNT,
    TA_CHUNK_NUM, WWF_CHUNK_NUM, TAG_TYPES_TO_KEEP,
    TO_PARQUET_CONFIG, TAG_GROUPBY_AGG, TAG_GROUPBY_LIST
)
...
def aggregate_works_tags_df(works_tags_df, minimum_work_count):
    ...
    works_with_fandom = works_with_fandom.rename(columns={'name_final': 'fandom_name'}).drop_duplicates()
    ...
    fandom_works_count = fandom_works_count.query(f'works_num > {minimum_work_count}')
    ...
    works_tags_df_no_fandom = works_tags_df.query('type_final != "Fandom"').drop(columns='tag_id')
